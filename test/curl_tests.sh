...
# grape-swagger exposes the OpenAPI (Swagger 2.0) spec of the live API;
# docs/openapi.json (generated by scripts/generate_openapi.rb) is its static
# snapshot served at /docs together with the interactive Scalar page.
SWAGGER_PATHS=$(curl -sS "$API/swagger_doc" | jq -r '.paths | length')
check_eq "swagger_doc documents all 6 endpoints" "6" "$SWAGGER_PATHS"
HTTP_CODE=$(curl -sS -o /dev/null -w '%{http_code}' "$BASE_URL/docs")
check_eq "interactive docs page served at /docs" "200" "$HTTP_CODE"
OPENAPI_PATHS=$(curl -sS "$BASE_URL/docs/openapi.json" | jq -r '.paths | length')
check_eq "/docs/openapi.json documents all 6 endpoints" "6" "$OPENAPI_PATHS"
